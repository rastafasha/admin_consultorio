...
export class OdontogramaComponent implements OnInit {
  ...
  guardarHallazgoEnServidor(dienteNum: number, cara: string, hallazgo: string) {
    const payload = {
      ...
      created_at: new Date().toISOString() // <-- Congelamos la hora exacta del diagnóstico clínico
    };

    // =========================================================================
    // DETECCIÓN DE CONECTIVIDAD (Estrategia de Sincronización Segmentada)
    // =========================================================================
    const isOnline = navigator.onLine;

    if (isOnline) {
      // -----------------------------------------------------------------------
      // MODO ONLINE: Envío HTTP directo normal a tu API de Laravel
      // -----------------------------------------------------------------------
      this.odontogramaService.guardarDiente(payload).subscribe((resp: any) => {
        if (resp && resp.status === 'success') {
          this.actualizarPantallaYProcesarBeep(dienteNum, cara, hallazgo);
        }
      });

    } else {
      // -----------------------------------------------------------------------
      // MODO OFFLINE: Resguardo inmediato por Pieza Dental (Costo $0 de servidor)
      // -----------------------------------------------------------------------
      const endpointPath = '/odontograma/guardar'; // El endpoint exacto de tu servicio de odontograma

      // Guardamos la pieza dental en la cola universal con la etiqueta 'Odontograma'
      this.offlineService.saveFormOffline(endpointPath, payload, 'Odontograma');

      // Actualizamos la interfaz visual de Angular localmente para que cambie de color el diente
      this.actualizarPantallaYProcesarBeep(dienteNum, cara, hallazgo);

      console.log(`[Klyntic Odontograma Offline] Guardada pieza ${dienteNum} (${hallazgo}) en caché local.`);
    }
  }

  /**
   * Helper modularizado para mantener sincronizada la interfaz gráfica 
   * y los AirPods tanto en modo Online como Offline.
   */
  private actualizarPantallaYProcesarBeep(dienteNum: number, cara: string, hallazgo: string) {
    const diente = this.odontogramaMapa.find(d => d.numero === dienteNum);
    if (diente) {
      if (!cara) {
        diente.hallazgo = hallazgo; // Estado general del diente (ej: Ausente)
      } else {
        this.actualizarCaraDienteMemoria(diente, cara, hallazgo); // Estado de la cara (ej: Caries Oclusal)
      }
      this.cdr.detectChanges(); // Forzamos a Angular a repintar el color gráfico del diente
      this.reproducirBeepOk();   // Emitimos el pitido electrónico de confirmación en los audífonos
    }
  }
    ...
    <tr style="border-bottom: 1px dashed #eee;">
      ...
      <td style="padding: 10px;">
        <span style="
          ...
          border-radius: 4px;
          color: #fff;
          background-color: ${diente.hallazgo === 'Caries' ? '#dc3545 !important' :
